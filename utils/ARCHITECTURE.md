...
- 放置跨模块复用的轻量工具。
- 管理本地化加载、路径计算、项目配置保存、全局配置存储、主题偏好和日志偏好。
- 提供分块、环境检测和状态管理等通用能力。
...
- `theme.py`：管理主题偏好，通过 `global_store.py` 写入根目录 `config.yaml`，并调用 qfluentwidgets 应用亮色、暗色或系统主题。
- `chunker.py`：预留文本或素材分块工具。
- `env_manager.py`：预留环境检测工具。
- `cloud_model_presets.py`：保存和读取云端模型配置预设，通过 `global_store.py` 写入根目录 `config.yaml`。
- `cloud_models.py`：拉取 OpenAI-compatible 云端模型列表。
- `llamacpp_downloader.py`：下载并安装 llama.cpp 运行时到 `bin/`。
- `logging_preferences.py`：管理日志等级偏好，默认使用 `INFO`，通过 `global_store.py` 写入根目录 `config.yaml`。
- `logging_middleware.py`：安装全局日志中间件。每次启动在 `log/` 中创建一个按启动时间命名的 `.log` 文件，并接收 Qt 消息和未捕获异常；日志等级跟随 `logging_preferences.py`，日志只写入文件；加载时清理旧日志，最多保留 20 个日志文件。
