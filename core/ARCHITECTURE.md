...
- `material_unit_scanner.py`：把文本、字幕、音频、图片和 GIF 映射为正式 run plan unit，负责唯一字幕关联与失败 warning、漫画/图集页组自然排序、稳定 ID、页码/章节 metadata、不支持状态和预处理来源 metadata 回填；只扫描 `materials/`，不扫描 cache、解析容器或执行模型提取。
