...
- `pages/project_page.py`：项目配置页。负责项目选择、提取模式、素材路径和素材处理配置；文件选择器只展示直接素材与已启用输入格式，异步整理结果汇总直接素材、容器派生材料、跳过视频和 warning。非原始方案包含直接视频但缺少 FFmpeg 时，页面提供取消、忽略全部视频继续、下载后自动执行三种选择；页面不解析容器，也不再编辑目标角色。
