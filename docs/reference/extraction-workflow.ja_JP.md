...
現在の安定実装は、主に動画素材の正式抽出、プレビュー抽出、キャラクターカードの基本コンパイル経路を対象にしています。ロードマップ 02 では正式抽出の初期化が run plan 構造に移り、トップレベルのメディア種別は `video`、`image`、`audio`、`text` の四種類に限定されています。テキスト、字幕、文字起こし結果、画像、漫画、混合メディアを統一されたプレビュー/ナレッジベース消費経路に入れる部分は、まだ今後の計画です。

## 1. 設計目標

CharaPicker の中心原則は `Extract Once` です。元の動画、音声、画像、字幕、テキスト素材は一度だけ解析し、その後は再利用できる構造化ナレッジベースとして保存します。
...
コード上のトップレベルメディア種別は `video`、`image`、`audio`、`text` のみです。アニメ、漫画、音声ドラマ、小説、字幕、設定集などは content form または metadata で表します。`transcript` は第五のメディア種別ではなく、`video` または `audio` から派生した `text` 型の中間成果です。
...
インポートと処理の後、システムはプロジェクトディレクトリ内に処理可能な素材を維持します。正式抽出の開始時に `materials/` を走査して `FormalExtractionRunPlan` を生成し、`knowledge_base/extraction_runs/{run_id}/plan.json` に保存します。run plan は `MaterialRef`、`ExtractionUnit`、メディア種別、content form、派生成果、内部 ID の対応を記録します。後続処理では `season_001`、`episode_001`、`chunk_0001` のような安定した ID を使い、元ファイル名から繰り返し推測しません。`source_manifest.json` は旧観測インデックスまたはデバッグ産物であり、正式抽出の入力契約ではありません。
...
```mermaid
flowchart TD
    A[元素材を raw/materials に入れる] --> B[materials を走査して素材 unit ビューを作る]
    B -.-> M[source_manifest.json デバッグ/旧観測インデックス]
    B --> C{モードを選択}
    C -->|プレビュー| P[先頭最大 2 個の動画 chunk を処理]
    P --> P1[preview__ chunk と preview__episode_content を書き込む]
    C -->|完全抽出| F[extraction_run_id と FormalExtractionRunPlan を作成]
    C -->|クリーン抽出| CL[再生成可能な中間産物を削除]
    CL --> F
    F --> RP[extraction_runs/{run_id}/plan.json を書き込む]
    RP --> L[season -> episode -> chunk の順に直列処理]
    ...
    Q --> FQ[extraction_run_id と FormalExtractionRunPlan を作成]
    FQ --> RQ[extraction_runs/{run_id}/plan.json を書き込む]
    RQ --> QC[文脈なしで chunk を並列抽出]
...
正式抽出は新しい `extraction_run_id` と `FormalExtractionRunPlan` を生成します。完全、クリーン、高速の各モードは、同じ run 内で schema が有効な full artifact だけを集約し、失敗後の再実行で古い結果が混ざることを防ぎます。
...
完全、クリーン、高速の各抽出は、毎回新しい `extraction_run_id` と run plan を生成します。run plan は正式抽出の主インデックスで、`knowledge_base/extraction_runs/{run_id}/plan.json` として保存されます。`chunk`、`episode`、`season` 産物にはその run id を記録し、後続の統合では現在 run 内で schema が有効な産物だけを読みます。これにより、失敗後の再実行で古い結果が混ざることを避けます。
...
- `source_trace` / `media_types`：産物がどの素材 unit とメディア種別に由来するかを記録し、動画、音声、画像、テキスト、派生成果を曖昧に混ぜないようにします。
- `derived_artifacts`：transcript など、元素材から派生した中間成果を記録します。transcript は `text` 型成果としてナレッジベースに入り、新しいトップレベルメディア種別にはなりません。
...
|   `-- {run_id}/
|       `-- plan.json
|-- source_manifest.json              # 旧観測インデックス/デバッグ産物
|-- seasons/
|   |-- season_001/
|   |   |-- season_content.json
|   |   |-- season_summary.json
...
キャラクターカードページの「目標会話グループ数」は、コンパイル成功時に必要な最終総数です。空欄ではモデルが自動決定し、0 では会話グループを保持せず、正数ではその数を厳密に満たします。初回結果が不足した場合、core は共通モデルミドルウェアを通じて、不足分だけを対象に証拠で制約した補完リクエストを一度行い、既存と追加の会話を重複排除します。それでも不足する場合は、件数不足のカードを成功として保存せず、コンパイルを明示的に失敗させます。
