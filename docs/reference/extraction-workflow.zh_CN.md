...
当前稳定实现主要覆盖视频素材的正式提取、预览提取和角色卡基础编译链路。文本、字幕、转写结果、图片、漫画和混合媒体进入统一预览/知识库消费路径仍属于后续计划。
...
导入和处理后，系统会在项目目录下维护可处理素材，并生成 `source_manifest.json`，记录原始文件夹、原始文件名和内部编号的映射。后续流程使用稳定编号，例如 `season_001`、`episode_001`、`chunk_0001`，不会反复依赖原始文件名推断。

预览链路当前只从 `materials/` 中最多收集前 2 个视频 chunk，写入 `preview__` 产物。预览不会消费正式提取产物，也不会把结果混入正式角色卡知识库。
...
```mermaid
flowchart TD
    A[原素材进入 raw/materials] --> B[扫描 materials 并生成 source_manifest]
    B --> C{选择模式}
    C -->|预览| P[最多处理前 2 个视频 chunk]
    P --> P1[写入 preview__ chunk 与 preview__episode_content]
    C -->|完整提取| F[创建正式 extraction_run_id]
    C -->|洁净提取| CL[清理可再生中间产物]
    CL --> F
    F --> L[按 season -> episode -> chunk 串行推进]
...
正式提取会生成新的 `extraction_run_id`。完整、洁净和快速模式只聚合同一 run 中 schema 合格的 full artifact，避免失败重跑时混入旧结果。
...
每次完整、洁净或快速提取都会生成新的 `extraction_run_id`。chunk、episode 和 season 产物会记录该 run id，后续合并只读取当前 run 中 schema 合格的产物，避免失败重跑时混入旧结果。
