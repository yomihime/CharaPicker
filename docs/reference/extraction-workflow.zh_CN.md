...
- `完整提取`：执行高质量线性流程，按 season -> episode -> chunk 顺序串行提取，并在每集、每季结束后用 AI 生成集级和季级产物。
- `洁净提取`：先清理可重新生成的提取中间产物，再执行与完整提取相同的高质量线性流程；已编译的正式角色卡会标记为需要重编译。
- `快速提取`：chunk 阶段按用户确认的并发数并行请求，不带上下文；全部 chunk 完成后，再用 AI 并发重整理 episode，最后并发重整理 season。这个模式速度优先，偏差会明显更大。
