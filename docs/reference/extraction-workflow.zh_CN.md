...
ZIP、CBZ、EPUB、文本型 PDF、7z、RAR 和 CBR 会先作为容器保存在 `raw/`，再派生为现有 text/image 等素材。派生路径镜像源文件在 `raw/` 下的相对路径，例如 `raw/groupA/book.zip` 和 `raw/groupB/book.zip` 分别写入 `materials/derived_inputs/groupA/book.zip/` 与 `materials/derived_inputs/groupB/book.zip/`；对应 manifest 写入 `cache/material_preprocessing/manifests/groupA/book.zip.json` 与 `groupB/book.zip.json`。内容变化会原子替换同一来源路径下的派生产物，不会改变寻址位置。

预处理 manifest 继续使用 schema v1，记录 `source_raw_path`、源后缀与 hash、preprocessor、输出根、派生 entry、计数/大小、warning 和失败项。每个派生 entry 已包含 `size_bytes` 与 `fingerprint`；复用、扫描映射和 raw 清理会校验实际文件大小及 SHA256 fingerprint，因此无需为完整性检查升级 manifest。
...
- `完整提取`：执行高质量线性补全流程，按 season -> episode -> chunk 顺序盘点正式产物。模型请求开始前，洞察流会显示 chunk 总数、可复用、缺失、失效、待提取数量，以及待更新的集/季聚合产物数量；盘点本身不改写已有产物。已有 JSON 必须可解析、身份与当前扫描一致、来源路径和可用指纹未变化；满足条件时直接复用，只对缺失、损坏或来源已变化的 chunk 调用模型。每个新提取 chunk 会带入结构化历史上下文；集/季产物输入未变化时同样复用，仅补齐缺失或受影响的聚合阶段。更换云模型不会自动令已有产物失效，需要全部使用新模型重做时应选择洁净提取。
