...
当前实现以 `FormalExtractionRunPlan` 作为正式提取主索引，顶层媒体类型固定为 `video`、`image`、`audio`、`text`。视频、普通文本、`.srt` / `.ass`、PNG/JPEG/WEBP 静态图片，以及由 audio/video 派生的 transcript 已有预览或正式提取路径；预览已支持通用 unit 成本采样，正式提取已通过内部分发表选择 video/text/image/audio transcript handler。漫画页组语义、原生音频/视频理解和跨媒体证据合并仍在后续里程碑中。
