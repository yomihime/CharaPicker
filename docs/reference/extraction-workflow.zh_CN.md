...
当前实现以 `FormalExtractionRunPlan` 作为正式提取主索引，顶层媒体类型固定为 `video`、`image`、`audio`、`text`。视频、普通文本、`.srt` / `.ass`、PNG/JPEG/WEBP 静态图片，以及由 audio/video 派生的 transcript 已有预览或正式提取路径；漫画页组语义、通用混合媒体调度、原生音频/视频理解和跨媒体证据合并仍在后续里程碑中。
...
预览链路当前优先尝试 `materials/` 中最多 2 个视频 chunk；没有可用视频结果时，依次回退到普通文本/字幕、音频 transcript 和静态图片 unit。预览写入隔离的 `preview__` 产物，不消费正式提取产物，也不会把结果混入正式角色卡知识库。跨内容形态的统一成本采样和 warning 调度仍由后续里程碑完善。
