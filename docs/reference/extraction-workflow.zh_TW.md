...
目前穩定實作主要覆蓋影片素材的正式提取、預覽提取和角色卡基礎編譯鏈路。路線 02 已把正式提取初始化改為 run plan 結構，並預留 `video`、`image`、`audio`、`text` 四種頂層媒體類型；文字、字幕、轉寫結果、圖片、漫畫和混合媒體進入統一預覽/知識庫消費路徑仍屬於後續計畫。

## 1. 設計目標

CharaPicker 的核心原則是 `Extract Once`：原始影片、音訊、圖片、字幕或文字素材只解析一次，之後沉澱為可複用的結構化知識庫。
...
程式碼層頂層媒體類型只允許 `video`、`image`、`audio`、`text`。番劇、漫畫、廣播劇、小說、字幕、設定集等屬於內容形態或 metadata；`transcript` 不是第五種媒體類型，而是從 `video` 或 `audio` 派生出的 `text` 型中間產物/成果。
...
匯入和處理後，系統會在專案目錄下維護可處理素材。正式提取開始時，系統會掃描 `materials/` 並生成 `FormalExtractionRunPlan`，寫入 `knowledge_base/extraction_runs/{run_id}/plan.json`。run plan 記錄 `MaterialRef`、`ExtractionUnit`、媒體類型、內容形態、派生成果和內部編號的映射；後續流程使用穩定編號，例如 `season_001`、`episode_001`、`chunk_0001`，不會反覆依賴原始檔名推斷。`source_manifest.json` 只作為舊觀察索引或調試產物，不再作為正式提取輸入契約。
...
```mermaid
flowchart TD
    A[原素材進入 raw/materials] --> B[掃描 materials 並建立素材單元視圖]
    B -.-> M[source_manifest.json 調試/舊觀察索引]
    B --> C{選擇模式}
    C -->|預覽| P[最多處理前 2 個影片 chunk]
    P --> P1[寫入 preview__ chunk 與 preview__episode_content]
    C -->|完整提取| F[建立 extraction_run_id 與 FormalExtractionRunPlan]
    C -->|潔淨提取| CL[清理可再生中間產物]
    CL --> F
    F --> RP[寫入 extraction_runs/{run_id}/plan.json]
    RP --> L[按 season -> episode -> chunk 串行推進]
    ...
    C -->|快速提取| Q[確認偏差風險與並發數]
    Q --> FQ[建立 extraction_run_id 與 FormalExtractionRunPlan]
    FQ --> RQ[寫入 extraction_runs/{run_id}/plan.json]
    RQ --> QC[chunk 並發提取且不帶上下文]
...
正式提取會生成新的 `extraction_run_id` 和 `FormalExtractionRunPlan`。完整、潔淨和快速模式只聚合同一 run 中 schema 合格的 full artifact，避免失敗重跑時混入舊結果。
...
每次完整、潔淨或快速提取都會生成新的 `extraction_run_id` 和 run plan。run plan 是正式提取的主索引，保存為 `knowledge_base/extraction_runs/{run_id}/plan.json`；`chunk`、`episode` 和 `season` 產物會記錄該 run id，後續合併只讀取目前 run 中 schema 合格的產物，避免失敗重跑時混入舊結果。
...
- `source_trace` / `media_types`：記錄產物來自哪些素材單元和媒體類型，避免把影片、音訊、圖片、文字或其派生成果混為同一種來源。
- `derived_artifacts`：記錄 transcript 等由原始素材派生出的中間產物。transcript 以 `text` 型成果進入知識庫，不作為新的頂層媒體類型。
...
|   `-- {run_id}/
|       `-- plan.json
|-- source_manifest.json              # 舊觀察索引/調試產物
|-- seasons/
|   |-- season_001/
|   |   |-- season_content.json
|   |   |-- season_summary.json
...
-> 根據角色卡身份欄位生成匹配名
-> 建構 direct / mention / causal / season_context 分層證據包
-> 若沒有 direct 證據，嘗試用 AI 從知識庫 targets 候選中解析已驗證別名
-> 用已驗證別名重建分層證據包
-> 沒有 direct 證據則失敗
-> 建構本地角色狀態 timeline
...
flowchart TD
    ...
    B --> C[建構 direct / mention / causal / season_context 分層證據包]
    C --> D{找到 direct 證據?}
    D -->|否| E[AI 從 knowledge_base targets 候選解析已驗證別名]
    E --> F{解析到可校驗別名?}
    F -->|是| B
...
角色卡頁面的「目標對話組數」表示編譯結果中的最終對話總組數：留空時由模型自動決定，0 時不保留對話組，正數則必須精確滿足。首次角色卡編譯返回不足時，core 會透過統一模型中介層發起一次只針對缺口、受證據約束的補充請求，並對既有與新增對話去重；補充後仍不足會明確使編譯失敗，不再把數量不足的角色卡儲存為成功結果。
...
目前基礎實作：角色卡 AI 複核輸入已經接入 `direct_evidence_episodes`、`mention_evidence_episodes`、`causal_context_episodes` 和 `season_context`。direct 證據由 episode 內容欄位中的角色名或已驗證別名命中形成；`targets` 只作為別名候選和輔助資訊，不單獨算 direct。mention、causal 和 season_context 用於補充動機、關係鏈和連續性，不能覆蓋 direct 證據。分層證據和品質評估寫入 `card.extensions["charapicker"]`，其中包含 `compile_evidence_layers`、`alias_resolution`、`needs_review_reasons`、`conflict_groups` 和 `parse_diagnostics`；普通 `quality.warnings` 只保留使用者可讀 warning，不直接暴露內部 reason key。
...
- 角色卡編譯上下文分層已接入基礎實作，但仍需繼續用真實素材驗收和調優 direct、mention、causal 與 season_context 的分類邊界。
