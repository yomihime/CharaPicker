...
4. [多素材平级接入前重构解耦执行计划](plans/02-multi-material-refactor-plan.zh_CN.md)：第二阶段架构体检与解耦计划，约束进入多素材覆盖前的重构边界。
5. [多素材种类支持执行计划](plans/03-multi-material-coverage-plan.zh_CN.md)：TODO P0 的第三阶段执行计划，要求路线 02 先准备好多素材接入矩阵。
