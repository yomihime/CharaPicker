# 更新包 Payload 识别修正完成记录

> 本文档原为执行计划；截至 2026-07-28，M01-M03 已按计划完成，保留为自更新 payload 识别修正的历史记录。
...
修复前实现通过 `UPDATE_ARCHIVE_ROOT = APP_NAME` 同时约束 ZIP 第一层名称和
...
### M01：拆分 ZIP 安全校验与 payload 解析（已完成）
