...
归档状态：本计划的开发生命周期已完成，并已随功能分支合并回主线。本文仅保留角色卡页面、角色卡编译、导入导出、预览草稿和 AstrBot 手动复制辅助的产品决策、里程碑拆分与验收记录。
...
以下内容保留当时的执行约束，用于理解该阶段为何这样拆分和验收；归档后不得直接按本文继续实施新功能。

- 本文原本是开发阶段执行计划。执行者按本文的已确认决策、模块边界、里程碑、提交分组和验收规则推进。
