...
> 归档状态：计划已于 2026-09-04 完成并通过自审查，相关改动经 [PR #39](https://github.com/yomihime/CharaPicker/pull/39) 合并。

## 完成记录

- 已实现 AstrBot 官方人格 JSON 映射、原子导出、预览、批量导出和编译后导出，并保留手动复制清单作为补充入口。
- 第一轮自审查发现空白显示名可能生成空 `persona_id`；修复为依次回退角色名和 `card_id`，补充回归测试后从完整验证和自审查重新开始。
- 最终验证通过：导出器目标单测 7 项、四语 i18n 校验、全仓 Ruff，以及统一离线回归 175 项测试。
- 最终整体自审查未发现剩余问题。
