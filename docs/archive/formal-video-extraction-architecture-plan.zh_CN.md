...
- 预览支线计划：[真实预览结果接入计划完成记录](preview-real-result-ingestion-plan.completed.zh_CN.md)
...
预览提取是正式提取计划的并行支线。详细计划见：[真实预览结果接入计划完成记录](preview-real-result-ingestion-plan.completed.zh_CN.md)。
...
- `i18n/*.json`
- `docs/archive/preview-real-result-ingestion-plan.completed.zh_CN.md`
...
- 改动文件：`core/knowledge_base.py`、`core/extractor.py`、`core/compiler.py`、`gui/main_window.py`、`i18n/*.json`、`docs/archive/preview-real-result-ingestion-plan.completed.zh_CN.md`、`docs/archive/formal-video-extraction-architecture-plan.zh_CN.md`
...
详细计划见：[真实预览结果接入计划完成记录](preview-real-result-ingestion-plan.completed.zh_CN.md)。
...
- 用户可以确认没有预览产物进入正式聚合。
- 用户可以明确选择是否允许单个服务商数据检查失败的 chunk 被跳过。
- 未解决风险被明确写出。
