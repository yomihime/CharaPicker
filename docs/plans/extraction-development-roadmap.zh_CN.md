...
可用性：可用作验收框架，不是当前剩余任务清单。实施任何条目之前，必须重新核对当前代码、`TODO.zh_CN.md` 和仍在 `docs/plans/` 中的专项计划。

本文面向项目开发者，保留 Extract Once、知识库和角色成长编译的长期目标与验收基准。它不是当前剩余任务清单；正式视频提取 MVP、角色卡最终编译基础生命周期、路线 01-03 和真实预览后续覆盖均已完成或归档，当前队列以 [TODO List](TODO.zh_CN.md) 为准。实施前必须重新核对当前代码。
...
- 初步提取只面向素材本身，不接收、使用或推断用户预设目标角色列表。
- 一次调用保留素材中出现的全部有效角色信息，避免为每个角色重复跑模型。
- 只根据输入素材和结构化上下文输出。
- 当前 chunk 新事实优先于历史摘要。
- 不编造输入中不存在的剧情或设定。
- 输出结构化 JSON。
