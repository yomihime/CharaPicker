...
计划阶段：草案，关键产品方向已确认；第 13 章当前无阻塞实施的产品决策。

可用性：本文是执行计划，不代表当前代码已实现。实施前仍需重新核对 `core/extractor.py`、`core/source_scanner.py`、`core/knowledge_base.py`、`core/models.py`、`gui/main_window.py`、`gui/pages/project_page.py`、`res/default_prompts.json` 和四个 `i18n/*.json`。
...
- 本轮新增可选 `context_window_tokens` 能力：用户可手动覆盖；应用可用内置模型能力表按 model id 推断；provider adapter 只有在供应商明确返回上下文窗口时才自动写入。三者都不可用时，标记 `context_window_unknown` 并使用保守默认预算。
...
- `run_full_extraction_streaming()` 会扫描 `materials/`，生成 manifest，逐个 chunk 独立调用 `formal_video_chunk_extraction`，再本地聚合 episode 和 season。
- `merge_episode_content()` 和 `merge_season_content()` 目前主要是本地去重拼接，不会把 chunk 或 episode 发给 AI 重新写合并内容。
- `load_current_season_episode_summaries()`、`load_previous_season_summary()` 和 `_build_chunk_payload()` 已存在，但正式视频提取路径没有使用它们。
- `ProjectConfig.include_previous_season_background` 当前没有被正式提取流程实际消费。
- 旧 full chunk JSON 可能在新一轮提取失败时继续被聚合读到，需要 run 隔离或洁净清理来避免脏数据。
...
当前仓库还没有模型上下文窗口配置；实施时新增可选 `context_window_tokens`。该值用于计算输入上下文和输出预留的总窗口，不等同于输出 token 上限。估算必须保守，尤其是中文、日文和 JSON 字段名不能按过低 token 成本估算。
...
- `context_window_tokens` 来源优先级：用户手动覆盖 > 内置模型能力表推断 > provider adapter 自动获取 > 保守默认预算。
...
- 增加可选 `context_window_tokens`，支持手动覆盖、内置模型能力表推断和 provider 尽力自动获取。
...
当前没有阻塞实施的产品决策。实现过程中如发现现有模型预设、供应商返回字段或 UI 空间不足以承载上述设计，再把具体冲突补回本章。
