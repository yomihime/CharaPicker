...
- 角色卡编译上下文还未拆成 `direct_evidence_episodes`、`mention_evidence_episodes`、`causal_context_episodes` 和 `season_context`；角色没有出现但解释行动动机的因果上下文仍可能过薄。
- `targets` 在角色状态编译中仍偏向门禁信号；后续应降级为相关性信号，避免因为候选索引不完整而丢上下文。
...
- 角色卡编译里的 `targets` 应从硬门禁降级为相关性信号，避免丢失未直接出现但解释动机的上下文。
