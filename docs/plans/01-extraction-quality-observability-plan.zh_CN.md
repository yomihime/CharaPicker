...
以下事项不是扩大 P1 范围的新需求；它们是执行时必须收口、降级或记录的已知不健全点。若无法在对应里程碑内低风险解决，应写入执行记录或路线 02 输入，不在 P1 临时做大范围重构。

- `max_output_tokens` 当前同时承担 UI 设置、视频每分钟换算、模型请求参数和输出指导语语义。M06 必须把用户可见语义改成“期望输出”，并为旧预设迁移或兼容读写留下清晰路径。
- 输出侧软期望与供应商 API 的 `max_tokens` 不是同一件事。执行时需要明确：用户看到的是软性输出期望；是否继续向后端传入硬性 `max_tokens`，属于中间件实现细节，不得在 UI 中承诺“精确控制输出 token”。
- `context_window_tokens` 已存在于云模型预设结构中，但模型页当前没有完整暴露保存/加载入口；`ExtractionRunBudgetPlan.episode_context_cap_tokens` 也存在默认 128000 的上下文纳入上限。P1 可以记录和接通最小必要链路，但不得把它扩展成新的输入策略系统。
- `期望图片输出 Token / 张` 与 `期望音频输出 Token / 分钟` 在 P1 只定义口径。路线 03 接入实际图片、漫画或音频消费前，不应把它们写成已生效提取能力。
- 角色卡编译使用独立期望输出 Token 会触达角色卡编译链路。执行时必须保持 `gui -> worker -> core` 边界，不让页面层直接拼接编译请求或知识库路径。
- prompt 拒绝样例的 JSON schema、zip 打包入口、开发者侧 `dev/refuse_samples/index.json` 结构、源码运行专属按钮和隐私确认文案仍需实现时复核。没有真实拒绝样例时，只固化流程和打包能力，不主动大改默认 prompt。
- token usage 展示的是供应商实际返回或归一化后的用量，不是用户配置项。UI 文案必须避免把 `prompt/completion/total` 和“期望输出 Token”混成同一概念。
...
执行顺序按“先离线可验证，再临时项目状态，再 UI 可观测性，最后拒绝样例双端流程与阶段闸门”推进。每个里程碑只承担一种主要风险：
...
| M06 | Token 设置弹窗和字段语义 | 不设计成本保护或新素材输入策略 |
| M07 | 拒绝样例用户侧记录与导出 | 不做开发者 prompt 更新 |
| M08 | 开发者样例导入与 prompt 建议 | 不自动应用默认 prompt 修改 |
| M09 | 第一阶段验收和路线 02 闸门 | 不把未完成 bug 塞进收尾 |
...
### M06：Token 设置弹窗与软期望语义

交付：

- 将模型页现有单行 `输出 Token / 分钟` 控件规划为“Token 设置”弹窗。
- 弹窗包含 `输入/上下文窗口 Token`、`期望视频输出 Token / 分钟`、`期望图片输出 Token / 张`、`期望音频输出 Token / 分钟`、`期望角色卡编译输出 Token`，并提供“恢复默认”“确认”“取消”。
- 验证输入侧硬约束和输出侧软期望只进入对应链路。
- 明确角色卡编译使用独立的期望编译输出 Token，不再复用视频每分钟输出期望。
- 对旧 `max_output_tokens` 预设读写保持兼容；如新增字段，执行时写清迁移和默认值来源。

验收：

- Token 设置弹窗能保存、恢复默认、确认和取消；取消不应改写预设。
- 弹窗明确提示：输入/上下文窗口是硬约束，期望输出 Token 是软性模型指令，不保证实际输出精确等于该值。
- 视频、图片、音频、角色卡编译和上下文窗口设置在 UI 文案和代码字段上不互相冒名。
- 角色卡编译请求不再使用视频每分钟输出期望；别名校验如保留内部 1024 以内小期望，需要在代码中有清晰注释或 helper 名称。
- 路线 03 才实际消费的图片/音频期望输出设置，不得在 P1 中写成已经完成的图片、漫画或音频提取能力。

边界：

- 不设计单次运行费用保护、图片输入策略、音频输入策略或多素材批处理策略。
- 不承诺应用能精确控制云端输出 token。
- 不让 GUI 页面层直接构造角色卡编译请求或知识库路径。
...
8. 更新流程从 `index.json` 中选中的样例开始：读取 `refusal_sample.json`，判断问题属于 prompt 语气、输出结构约束、模型后端策略、期望输出 token 不足，还是素材内容本身。
9. 若属于用户项目特定需求，优先生成用户 prompt override 建议。
10. 若确认具备通用价值，再由 `dev/regeneration_prompt/` 生成默认 prompt patch 建议；默认 prompt 的实际修改仍需开发者确认。
11. 修改后用相同脱敏样例或用户允许的打包素材复跑，并确认正式提取 JSON 可解析。
...
- 拒绝样例流程已确认用户侧本地记录、本地打包、主动回传，以及开发者侧 `dev/refuse_samples/` 导入、同名解压和 `index.json` 索引维护；不存在自动上传路径。
...
### G04：进度反馈、Token 设置与文案修正

覆盖：

- M05 中需要代码修正的进度、token usage、失败反馈或洞察事件问题。
- M06 中需要代码修正的 Token 设置弹窗、字段语义、旧预设兼容和角色卡编译期望输出拆分。
...
- M07 中用户侧本地样例记录、zip 打包、轻量 hash 和打包入口。
- 如有 UI 文案变化，同步 `i18n/*.json`。
...
- M08 中开发者侧 `dev/refuse_samples/` intake、同名解压、`index.json` 维护和 `dev/regeneration_prompt/` 工具链。
- M09 第一阶段完成小结或 TODO 状态同步。
