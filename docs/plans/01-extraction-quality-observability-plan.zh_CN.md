...
- prompt 拒绝样例的 JSON schema、zip 打包入口和隐私确认文案仍需实现时复核。没有真实拒绝样例时，只固化流程和打包能力，不主动大改默认 prompt。
...
执行顺序按“先离线可验证，再临时项目状态，再 UI 可观测性，最后 prompt 与阶段闸门”推进。每个里程碑只承担一种主要风险：
...
| M07 | prompt 拒绝样例维护流程 | 不自动上传，不提交用户样例 |
| M08 | 第一阶段验收和路线 02 闸门 | 不把未完成 bug 塞进收尾 |
...
- 拒绝样例流程已确认本地记录、本地打包、用户主动回传，不存在自动上传路径。
...
- M07 中因真实拒绝样例产生的 prompt 修改或本地样例打包流程。
- M08 第一阶段完成小结或 TODO 状态同步。
...
6. 若近期有安全拒绝样例，检查本地样例打包路径、复跑对应样例并确认 JSON 输出约束。
