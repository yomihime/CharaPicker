...
Authenticode 证书取舍，以及最终 `v1.0.0` tag 与 GitHub Release 授权。满足这些条件前不归档本计划。
...
| B06 | `release/v1-docs-text` | 第一部分：文档与文本 + 最终集成 | M01、M09 和 M10 的可自动执行部分；计划归档和稳定版本元数据受人工门槛约束 | 执行中 |
...
Prompt 提交是条件项：只有 B03 已提供明确分类且存在可复现的合法误拒绝证据时才执行；没有证据就不做语义性改写，并在 Release 记录中说明。计划归档只在 M01-M15、候选包验收和用户试用完成后执行；版本提升提交必须最后发生，且提交前所有自动门禁通过。当前 B06 只完成可自动执行的收口，不在缺少用户验收和稳定版发布授权时提前归档计划或提升为 `v1.0.0`。
