...
- 当前检查点：`v1.0.0-rc.2` 已于 2026-08-16 发布；B00-B07 已合并并删除分支，稳定版发布已获授权。
...
| B07 | 已完成（PR #30、Actions #33260200101） | 稳定版文本与版本元数据已进入 `main`，分支已删除；官方 Release 锁候选构建与打包态健康检查通过。 |

维护者已于 2026-08-30 明确接受跳过 RC.3、缺少新增真人 Windows/UI 与真实 Prompt 样例证据、
以及未使用 Authenticode 的分发风险，并授权创建、推送 `v1.0.0` tag 和发布 GitHub Release。这表示接受剩余风险，不表示这些人工项目已经通过；
...
| B07 | `release/v1.0.0` | 稳定版准备 | 稳定版状态文本、CHANGELOG、版本元数据、最终门禁与候选包验证 | 已合并 PR #30，分支已删除 |
...
Prompt 提交是条件项：只有 B03 已提供明确分类且存在可复现的合法误拒绝证据时才执行；没有证据就不做语义性改写，并在 Release 记录中说明。B06 已完成可自动执行的收口；维护者于 2026-08-30 接受剩余人工风险，B07 已完成并获稳定版发布授权。计划在稳定版发布链路完成后归档。
...
- B06 与 B07 已合并并删除，维护者已授权创建、推送 `v1.0.0` tag 并发布 GitHub Release。
