...
- 计划阶段：`v1.0.0-rc` 到 `v1.0.0` Release 的待执行专项。
- 可用性：可作为后续实现入口；执行前仍需复核当前代码、依赖版本、GitHub Actions 和证书条件。
- 最近整理日期：2026-08-15。
...
| 主题 | 当前状态 | Release 目标 |
| --- | --- | --- |
| 用户文档 | 应用、构建和 tag 已是 `1.0.0-rc`，四语 README 仍声明 beta；`AGENTS.md` 仍写 `v0.5.0-beta` | 四语状态一致，准确描述 1.0 边界、限制、数据和签名状态 |
| 数据耐久性 | `core.knowledge_base.write_json()`、项目配置和全局配置直接覆盖写入；素材预处理已有局部原子写入实现 | 关键事实和可再生产物统一使用经过测试的原子写入；关键事实具备有限恢复路径 |
| 构建可复现性 | `requirements.txt` 多为开放下界，CI 安装当时最新 PyInstaller，Python 只固定到 `3.12`，runner 与 action tag 会漂移 | 官方 Release 使用锁定依赖、明确 Python patch、可审计 action 版本和无敏感信息的构建清单 |
| 签名与分发信誉 | 发布 zip 有 SHA-256；当前 tag/commit/Windows 二进制没有形成完整签名链 | 至少形成来源、hash、构建溯源和未签名披露；有证书时加入 Authenticode 与时间戳 |
| Prompt 安全误拒绝 | 已有默认 prompt、用户 override、脱敏失败样例和拒绝样例打包基础；缺少稳定的基线指标与版本归因 | 建立分类、基线、定向调优、复跑和回归记录闭环，不牺牲 JSON 与证据质量 |
| 全自动验证 | 统一离线回归与 49 个单测本地通过，但发布 workflow 不运行 Ruff 或回归；没有发布包契约、打包态健康检查和外部下载完整性门禁 | PR、主分支和 tag 使用分层门禁；文档与文本、构建产物和优化项均有确定的自动验收 |
...
| 顺序 | 分支 | 所属部分 | 覆盖内容 |
| --- | --- | --- | --- |
| B00 | `docs/rc-release-plan` | 计划固化 | 只提交本计划及其索引；这是实施前的计划 PR，不计入第一部分产品文档分支 |
| B01 | `release/v1-automated-validation` | 第二部分：全自动验证 | M04、M05、M11、M12、M14、M15 |
| B02 | `release/v1-data-durability` | 第三部分：优化与完善 | M02、M03 |
| B03 | `release/v1-refusal-classification` | 第三部分：优化与完善 | M08 |
| B04 | `release/v1-download-hardening` | 第三部分：优化与完善 | M13 |
| B05 | `release/v1-distribution-trust` | 第三部分：优化与完善 | M06、M07 |
| B06 | `release/v1-docs-text` | 第一部分：文档与文本 + 最终集成 | M01、M09、M10、计划归档和最终版本元数据 |
...
Prompt 提交是条件项：只有 B03 已提供明确分类且存在可复现的合法误拒绝证据时才执行；没有证据就不做语义性改写，并在 Release 记录中说明。计划归档只在 M01-M15、候选包验收和用户试用完成后执行；版本提升提交必须最后发生，且提交前所有自动门禁通过。
...
- B06 合并并删除后，仍需维护者明确授权才能创建 `v1.0.0` tag 和发布 GitHub Release。
