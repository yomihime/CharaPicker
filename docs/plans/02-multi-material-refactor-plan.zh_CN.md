# 第二阶段：多素材平级接入前重构解耦执行计划（zh_CN）

最近整理日期：2026-06-09

计划状态：正式执行计划，执行前仍需按当前代码复核。

适用阶段：路线 02，在路线 01 提取质量与可观测性计划打磨完成之后、路线 03 多素材覆盖实施之前执行。本计划不假设路线 01 的所有实现已经落地；真正执行路线 02 时，必须先重新核对当时的代码、配置、架构文档和路线 01 已完成项。
...
第二阶段的目标不是立刻支持所有素材，而是在接入更多素材前先把结构拆清楚：让视频成为一种素材类型，而不是整条 Extract Once 工作流的隐含前提。

完成后，后续路线 03 应能在不重写主提取流程的前提下，把文本、字幕、音频转写、图片、漫画和混合媒体按平级素材类型接入。

## 2. 范围

本计划覆盖多素材接入前的架构体检、耦合拆分、内部边界固化和回归验证。
...
- 为路线 03 预留文本、字幕、音频、图片、漫画、混合媒体的接入口和验收口径。

## 3. 不做范围

- 不在本阶段实现完整文本、字幕、图片、漫画或混合媒体提取功能。
...
- 每个里程碑只拆一个清晰边界，避免把 UI、数据模型、IO、API、迁移和多素材功能塞进同一阶段。
- 内部抽象必须服务当前可见复杂度，不能为了“以后也许需要”堆空接口。
- 视频适配应成为第一种被迁移到平级入口的素材类型，用它证明新边界能保持旧行为。
- 音频转写在 02 中先被明确为“可被引用的派生素材/文本上下文”，独立音频提取能力留给 03。
- 文本、字幕、图片、漫画在 02 中只定义接入位置、数据契约和测试桩，不做完整产品能力。
- 02 的目标架构必须给路线 03 的新媒体接入做好准备，并交付多素材接入准备矩阵。该矩阵至少覆盖文本、字幕、音频转写、原生音频理解、图片、漫画和混合媒体的素材类型、提取单元、预算口径、模型能力、证据引用、预览入口、正式入口、知识库落点和失败反馈。
...
- 预算缩放在 chunk 提取附近计算，缺少按素材类型分派的统一接口。
...
4. `core` 按素材类型选择内部处理器，构建模型请求或本地派生结果。
...
| 概念 | 含义 | 约束 |
| --- | --- | --- |
| MaterialKind | 素材类型，如 `video`、`audio`、`transcript`、`text`、`subtitle`、`image`、`manga`、`mixed` | 用于扫描、预算、模型能力和知识库 metadata，不等于 UI 文件来源类型 |
| MaterialRef | 指向 `materials/` 中一个原始或处理后素材的稳定引用 | 应优先使用相对路径，避免写死绝对路径 |
| ExtractionUnit | 一次可被提取的最小工作单元 | 视频可以是 chunk，文本可以是段落/章节，图片可以是单张或页，漫画可以是页组 |
...
| DerivedMaterial | 从素材生成的中间材料，如 transcript、抽帧、OCR 文本 | 不能无条件等同于正式提取结果 |
...
- 旧 manifest 没有 `schema_version` 或 `material_kinds` 时，按当前视频 manifest 读取。
...
- 通用层只认识 `ExtractionUnit`、`MaterialKind`、预算、上下文、产物模型。
- 视频处理器包住现有视频 chunk 请求构建、时长探测、输入模式、转写上下文逻辑。
- 文本/字幕/图片/漫画处理器在 02 中最多保留接口或测试桩，不做正式 UI 开关。
...
路线 02 的重构目标不能只解决视频拆耦，还必须让路线 03 接入新媒体时不用再次移动主架构。

执行 M02 和 M15 时必须维护一张多素材接入准备矩阵：

| 素材类型 | 02 必须准备的架构位置 | 不在 02 中实现的内容 |
| --- | --- | --- |
| 文本 | `MaterialKind=text`、文本 unit、文本 evidence、text backend 分派、文本预算占位 | 文本正式提取 prompt 和完整 GUI 能力 |
| 字幕 | `MaterialKind=subtitle`、时间轴 evidence、subtitle/transcript-like unit、与 episode 对齐的 metadata | 字幕解析细节和说话人推断 |
| 音频转写 | `MaterialKind=transcript`、派生素材关系、episode transcript 引用、转写失败状态 | 独立音频知识库提取能力 |
| 原生音频理解 | `MaterialKind=audio`、audio backend 能力判断、音频时间段 evidence、不可用状态 | 原生音频正式提取请求 |
| 图片 | `MaterialKind=image`、image unit、图片路径/序号/区域 evidence、图片输出期望入口 | 图片视觉提取 prompt 和批处理 |
| 漫画 | `MaterialKind=manga`、页/页组 unit、页码 evidence、漫画目录排序占位 | 漫画页理解、OCR 或跨页聚合 |
| 混合媒体 | 单 episode 多 material kinds、多个 evidence refs、冲突/重复事实合并占位 | 混合媒体正式聚合策略 |
...
如果某类素材在 02 结束时无法填完整，必须在 02 收尾报告中列为路线 03 前置阻塞，而不是留给 03 临时绕开。
...
- 在 `core/models.py` 或新的小型 core 模块中定义素材类型、素材引用、提取单元、派生素材和证据引用的模型。
...
- 新模型字段有默认值或兼容读取路径。
- 模型名称表达素材语义，不把 `video_path` 当作通用字段。
...
- 保留 `chunks` 字段读取。
- 允许写入兼容字段，如 `schema_version`、`material_kinds`、`units`，但不能让旧流程必须依赖新字段。
- 明确 `source_kind`、`material_kind`、`unit_kind` 的关系。
...
- 将 mode、run id、stage、budget plan、source manifest、season/episode refs、material kinds 放入计划。
...
- 建议新增或规范 `material_kind`、`source_counts`、`context_policy`、`requested_output_tokens` 的写入策略。
...
- 规划通用 meta，如 `material_kind`、`unit_id`、`source_path`、`stage`。
...
- 更新 `docs/plans/TODO.zh_CN.md` 或后续 03 文档，列出可直接接入的素材类型入口。
- 交付多素材接入准备矩阵，逐项说明文本、字幕、音频转写、原生音频理解、图片、漫画和混合媒体是否已经具备路线 03 接入条件。
...
| 预算语义混乱 | 非视频素材继续使用“每分钟” | 通用层只接受按素材类型解析后的预算 |
...
- `ChunkExtractionResult` 是否足够表达图片、文本、漫画的未来最小单元。02 只能给出兼容方向，不宜一次性扩展到所有领域字段。
- subtitle 应归为 `text` 的子类、独立 `subtitle`，还是 transcript 派生材料。建议 02 先在 `MaterialKind` 中保留清晰位置，03 根据实现成本决定。
- manga 是图片集合、页组还是独立素材类型。建议 02 只定义 `manga` kind 和 evidence 需要页号/区域，真实 OCR/视觉策略留给 03。
- mixed media 的单位边界。建议 02 不实现，只要求 manifest 和运行计划能表达一个 episode 中多个 material kinds。
...
只有满足以下条件，才建议进入多素材覆盖计划：

- 视频链路已经通过通用扫描/运行计划或等价边界运行。
- 新旧 manifest 兼容策略明确。
- 素材类型、提取单元、派生素材、证据引用的内部契约稳定。
- 预算接口不再强迫非视频素材使用视频每分钟语义。
- transcript 已有明确派生素材边界。
- 多素材接入准备矩阵已经覆盖文本、字幕、音频转写、原生音频理解、图片、漫画和混合媒体，并明确每类素材的预览入口、正式入口、模型能力、预算口径、知识库落点和失败反馈。
- GUI/worker/core/utils 的责任没有因重构变得更混乱。
- 文档中列出路线 03 可以直接接入的入口和仍需实现的素材处理器。

路线 03 的首要任务应是基于 02 边界补真实素材覆盖，而不是再次大规模移动架构。
