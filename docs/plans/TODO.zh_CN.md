...
| 顺序 | 待办 | 优先级 | 规模 | 主要验收点 |
| --- | --- | --- | --- | --- |
| 1 | 支持 EPUB 小说与漫画压缩包的受控导入预处理，扩展 PDF、7z 等更多输入格式计划见 [更多输入格式支持执行计划](input-format-support-plan.zh_CN.md) | 中 | 中 | 明确 EPUB 正文/章节/插图提取与漫画 ZIP/CBZ/RAR/7z 解包边界，防止路径穿越和解压炸弹，保留原容器到派生 TXT/图片目录的来源追踪；当前 `.epub` 为 unknown，漫画压缩包明确 unsupported；PDF 和 7z 需先确认依赖或外部工具策略。 |
