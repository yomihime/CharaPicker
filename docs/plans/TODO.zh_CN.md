...
| 顺序 | 待办 | 优先级 | 规模 | 主要验收点 |
| --- | --- | --- | --- | --- |
| 1 | 持续运行统一离线回归和打包验收 | 高 | 小/持续 | 保持四媒体类型、预览/正式隔离、来源追踪、旧视频链路、自更新包结构和打包态运行根目录不回退；正式构建继续在 PyInstaller 前检查 Release 锁和目标工具链，并隔离宿主机 `PATH`；发布前按 [打包与发布规范](../reference/release-packaging.zh_CN.md) 在最新 `main` 上手动运行一次 Windows Build。 |
