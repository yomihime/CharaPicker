...
路线 01 的提取质量基线、路线 02 的正式提取架构重构，以及路线 03 的多内容形态离线实现已经按顺序完成。当前执行重点调整为：

1. 持续运行统一离线回归，保持四媒体类型、预览/正式隔离、来源追踪和旧视频链路不回退。
2. 在用户明确授权真实素材和模型费用后，补齐路线 03 的非视频真实模型验收矩阵。
3. 根据真实验收结果继续调整供应商拒绝、证据质量、进度和 UI 反馈，不提前扩展新的媒体类型或大范围插件架构。

## P0：Extract Once 覆盖缺口

路线 03 的扫描、handler、预览、正式聚合、角色卡证据、失败记录、GUI 状态与统一离线回归已经落地，执行事实见 [番剧、漫画、广播剧、小说等内容形态支持执行计划](03-multi-material-coverage-plan.zh_CN.md)。当前 P0 只保留尚未完成的真实模型验收。

| 顺序 | 待办 | 优先级 | 规模 | 主要验收点 |
| --- | --- | --- | --- | --- |
| 1 | 补齐多内容形态真实模型验收 | 高 | 中 | 在用户明确授权的真实项目上分别验证小说/设定文本、字幕/台本、独立音频转写、图片/插画、漫画目录和视频 + 字幕/台本的预览、正式提取、知识库产物和角色卡消费；只读预检通过不能替代真实验收。视频项目子集已于 2026-06-22 通过。 |
...
- 路线 03 视频真实验收子集完成：真实预览、正式提取、供应商拒绝降级、当前 run 聚合和角色卡重编译均已验证；非视频真实素材矩阵仍保留在 P0。
