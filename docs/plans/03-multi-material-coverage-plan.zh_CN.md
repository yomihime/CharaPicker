...
- 混合资料包：由 `video`、`image`、`audio`、`text` 四种媒体类型组合，不新增 `mixed` 顶层媒体类型。
...
混合资料包指同一项目、season 或 episode 下存在多种内容形态，并由 `video`、`image`、`audio`、`text` 四种媒体类型组合表达，例如番剧视频 + 字幕/台本 + 漫画页 + 小说/设定文本。

处理目标：

- 用统一运行计划表达一个 episode 中多个 media types 和 content forms。
...
| 混合资料包（四种媒体类型组合） | 多个 evidence refs 的集合和冲突说明 |
...
- 方案 A：新增小说/设定文本、字幕/台本和转写文本提取的软性输出期望设置，并同步 i18n、默认值、规整 helper 和提示说明。
...
- 明确 `.json`、`.gif`、漫画压缩包、未知扩展名的处理策略，并说明它们映射到哪种媒体类型或是否需要用户选择内容形态。
...
- 漫画目录映射为带页码/页组 metadata 的 `image` unit。
- 混合目录映射为一个 episode 下的多 media type / content form unit 集合。
...
### M10：混合资料包 episode 编排

目标：同一 episode 中多种媒体类型和内容形态可协同提取。
...
- 番剧 + 字幕/台本、图片 + 设定文本、漫画 + 小说/设定文本等组合可表达。
- 聚合结果保留多证据来源。
- 冲突不被静默覆盖。
...
- `feat: merge multi-form content episodes`
  - 混合资料包 episode 聚合和冲突处理。
...
- 小说/设定文本、字幕/台本、音频转写、图片、漫画和至少一种混合资料包组合可以进入预览。
