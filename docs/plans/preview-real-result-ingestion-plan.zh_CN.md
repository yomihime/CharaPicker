...
- 在已完成角色卡页面和正式知识库编译链路基础上，继续强化按季、集推进的 timeline 可读性。
- 加入更明确的冲突识别、证据优先级、质量警告和 `needs_review` 判断。
- 明确是否需要把共享阶段状态缓存写入 `character_stage_states.json`；若需要，作为独立缓存设计，不替代 `character_cards/{card_id}/card.json` 母本。
- 让角色卡质量打磨继续优先读取结构化知识库，而不是重新分析原始素材。

验收标准：

- 单个角色卡能展示或导出按集推进的阶段状态。
- 冲突事实不会被静默覆盖，必须保留证据来源或处理说明。
- 质量警告、需要复核和部分成功状态对用户清楚可见。
