...
> 状态提醒：本文是阶段性接入计划，部分描述已被当前代码推进或替代。当前 `core/extractor.py` 已开始从 `materials/` 中收集视频 chunk，并通过云端模型生成/保存 chunk JSON；本文中“当前仍使用项目元数据占位”和“第一阶段只接入文本类素材”的描述不可直接当作当前事实。实施相关任务前必须重新核对 `core/extractor.py`、`gui/main_window.py` 和 `AGENTS.md` 的来源优先级。
