...
当前预览会刻意忽略已经存在的 chunk JSON，不把旧的预览结果作为本轮预览输入；每次预览都从 `materials/` 中的可消费视频 chunk 重新发起提取，以便反映当前素材、模型预设和输出 Token / 分钟设置。这是当前阶段的预期行为，不视为 Extract Once 复用缺口。

正式提取流程是否复用或忽略预览生成内容尚未定案。后续评估 full extraction 时，可能需要让正式提取明确忽略预览阶段生成的 chunk/episode 内容，避免预览试算污染正式知识库；也可能通过标记或隔离预览产物解决，待评估后再定。
...
- 预览阶段当前不复用既有 chunk JSON；它重新从 `materials/` 生成预览结果，确保试跑反映当前素材与模型设置。
- 正式提取是否忽略预览生成内容待评估；若需要，后续应增加预览产物标记或独立命名空间。
...
- 即使 `knowledge_base` 已有可读 chunk JSON，当前预览仍按预期从 `materials/` 重新提取，而不是直接复用旧 JSON。
- 初步提取生成的 chunk JSON 不绑定用户预设目标角色，后续角色卡生成再基于目标角色读取和过滤结构化结果。
- 后续评估正式提取时，要明确预览生成内容是否需要被 full extraction 忽略，避免预览试跑结果影响正式知识库产物。
