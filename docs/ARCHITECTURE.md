...
- `plans/extraction-quality-observability-plan.framework.zh_CN.md`：提取质量与可观测性的第一阶段计划框架，只作为正式计划起草入口。
- `plans/multi-material-refactor-plan.framework.zh_CN.md`：多素材平级接入前架构体检与解耦的第二阶段计划框架，只作为正式计划起草入口。
- `plans/multi-material-coverage-plan.framework.zh_CN.md`：文本、字幕、音频转写、图片、漫画和混合媒体支持的第三阶段计划框架，只作为正式计划起草入口。
